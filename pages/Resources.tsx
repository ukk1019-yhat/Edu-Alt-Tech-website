...
import { auth, onAuthStateChanged, db, collection, getDocs, addDoc, serverTimestamp } from '../lib/firebase';
...
const SEED_RESOURCES: ResourceItem[] = [
  { title: 'Complete Guide to Calculus', description: 'Derivatives, integrals, and limits explained with step-by-step examples for engineering students.', type: 'pdf', category: 'Mathematics', premium: false, downloads: '2,341', classLevel: 'College/Engineering' },
  { title: 'Python for Data Science', description: 'Comprehensive Python notes covering NumPy, Pandas, Matplotlib, and basic ML algorithms.', type: 'notes', category: 'Computer Science', premium: false, downloads: '1,892', classLevel: 'College/Engineering' },
  { title: 'Physics Formula Sheet', description: 'Complete formula reference for mechanics, electromagnetism, thermodynamics, and modern physics.', type: 'pdf', category: 'Science', premium: false, downloads: '3,104', classLevel: 'JEE Main' },
  { title: 'JEE Main Previous Year Papers', description: 'Compiled PYQs from the last 5 years with detailed solutions for all three subjects.', type: 'pdf', category: 'Mathematics', premium: true, downloads: '4,557', classLevel: 'JEE Main' },
  { title: 'Organic Chemistry Reactions', description: 'Curated list of named reactions, mechanisms, and reagents for quick revision.', type: 'notes', category: 'Science', premium: false, downloads: '1,234', classLevel: 'College/Engineering' },
    ...
    const fetchResources = async () => {
      try {
        ...
        if (items.length === 0) {
          ...
          if (seeded.length > 0) setFirebaseResources(seeded);
        } else {
          setFirebaseResources(items);
        }
      } catch (e) { console.error('Resources: Failed to fetch resources from database', e); }
    };
    fetchResources();
  }, []);
